...
# option 2: creating a new categorical variable using mutate and case_when
# commands
...
pkir |> select(starts_with("b16")) |> vtable()

print_labels(pkir$b16_03)

pkir |> select(starts_with("b16"))

pkir |> dim()
               
library(haven)

TR <- read_dta("C:/Users/92300/Downloads/TRIR51FL.DTA")

library(tidyverse)
TR |> dim()
library(labelled)
library(vtable)
TR |> select(starts_with("w")) |> vtable()

TR |> select(w110,w112) |> filter(w110!=8 & w112!=8, w110>0, w112>0)


TR13 <- read_dta("C:/Users/92300/Downloads/TRIR62FL.DTA")
