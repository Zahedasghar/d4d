...
#' ## Example
#' 
#' **Question:** Filter to students who are male and went to rural.
#' 
#' 
scores |> filter(school=="rural")
...
scores |>
  summarize(reading_score_mean = mean(reading_score), sd=sd(reading_score))
...
#' 
#' **Answer:** They both get the max math score by teacher for teachers at rural school. 
#' The first block calculates the max_math_score for both urban and rural and then filters 
#' out urban after that calculation. The second block filters out urban right away. 
